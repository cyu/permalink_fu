...
module PermalinkFu
  ...
  class << self
    ...
    def escape(string, seo_elim = false)
      ...
      if (seo_elim.is_a?(TrueClass) || seo_elim.is_a?(Hash))
        seo_elim = {} unless seo_elim.is_a?(Hash)
        seo_elim[:min_length] ||= 50
        if seo_elim[:words] && seo_elim[:words].is_a?(Array)
          seo_elim[:words] = Regexp.new("(^| )(#{seo_elim[:words].join('|')})($| )", true)
        else
          seo_elim[:words] = SEO_WORD_REGEX        
        end
      else
        seo_elim = false
      end      
      result.gsub!(seo_elim[:words], ' ') if (seo_elim && (result.size > seo_elim[:min_length]) && seo_elim.is_a?(Hash))
  ...
  module PluginMethods
    ...
    def has_permalink(attr_names = [], permalink_field = nil, options = {})
      ...
      ClassMethods.setup_permalink_fu_on self do
        ...
        self.permalink_options    = {:unique => true, :strip_words => false}.update(options)
      end
    end
  end
  ...
  module ClassMethods
    ...
    def define_attribute_methods_with_permalinks
      if value = define_attribute_methods_without_permalinks 
        evaluate_attribute_method permalink_field, "def #{self.permalink_field}=(new_value);write_attribute(:#{self.permalink_field}, new_value.blank? ? '' : PermalinkFu.escape(new_value, self.class.permalink_options[:strip_words]));end", "#{self.permalink_field}="
      end
      value
    end
  end
